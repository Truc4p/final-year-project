...
These test cases document manual testing performed on the WrenCOS e-commerce platform. All endpoints are tested via the REST API with base URL `http://localhost:3000`.
...
| AUTH-01 | Missing Token Access Denied | 1. Send POST request to `/products` without Authorization header.<br>2. Include valid product data in request body. | 401 status with message "No token, authorization denied". | 401 returned with the specified message. | Pass |
| AUTH-02 | Invalid Token Access Denied | 1. Send POST request to `/products` with header `Authorization: Bearer invalidtoken`.<br>2. Include valid product data. | 401 status with message "Token is not valid". | 401 returned with invalid token message. | Pass |
| AUTH-03 | Forbidden Role for Product Create | 1. Authenticate as customer user.<br>2. Send POST request to `/products` with valid product data. | 403 status with message "Access denied". | 403 returned as expected. | Pass |
| **Product Management** |
| PROD-01 | Create Product with Image Upload | 1. Authenticate as admin user.<br>2. Send POST request to `/products` with multipart/form-data including image file and fields (name, price, category, ingredients, skinType, benefits, tags, skinConcerns). | 201 status with product JSON; array fields parsed correctly; image path set under `/uploads/`. | 201 returned; arrays populated; image saved and accessible. | Pass |
| PROD-02 | Update Product Replaces Image | 1. Authenticate as admin user.<br>2. Send PUT request to `/products/{id}` with new image and updated fields.<br>3. Verify old image file removed from disk. | 200 status with updated product; old image deleted from disk. | 200 returned; old image removed after successful update. | Pass |
| PROD-03 | Get Product Not Found | 1. Send GET request to `/products/{nonexistentId}`. | 404 status with message "Product not found". | 404 returned with error message. | Pass |
| PROD-04 | Delete Product Removes Image | 1. Authenticate as admin user.<br>2. Send DELETE request to `/products/{id}`.<br>3. Attempt to access previous image path. | 204 No Content status; image file no longer available on disk. | 204 returned; image file removed. | Pass |
| PROD-05 | List Products Public Access | 1. Send GET request to `/products` without authentication. | 200 status with array of all products. | 200 returned with product array. | Pass |
| **File Upload** |
| UPLOAD-01 | Upload Thumbnail Success | 1. Authenticate as admin user.<br>2. Send POST request to `/uploads/thumbnail` with JPEG file < 5MB in field "thumbnail". | 200 status with response containing filename, path `/uploads/thumbnails/{file}`, and file size. | 200 returned with metadata. | Pass |
| UPLOAD-02 | Upload Thumbnail Invalid Type | 1. Authenticate as admin user.<br>2. Send POST request to `/uploads/thumbnail` with GIF file. | 500 status with error message "Only image files are allowed for thumbnails". | 500 error with message "Only image files are allowed for thumbnails". | Pass |
| UPLOAD-03 | Upload Thumbnail Exceeds Size | 1. Authenticate as admin user.<br>2. Send POST request to `/uploads/thumbnail` with file > 5MB. | 500 status with error due to Multer size limit. | 500 error returned by Multer size constraint. | Pass |
| **Livestream** |
| LIVE-01 | Get Active Livestream | 1. Send GET request to `/livestreams/active`. | 200 status with active livestream metadata when one is active; null otherwise. | Active livestream returned with isActive: true. | Pass |
| LIVE-02 | Create Livestream (Admin Only) | 1. Send POST request to `/livestreams` as admin with valid payload.<br>2. Repeat as customer user. | 201 status for admin; 403 status for customer. | 201 for admin; 403 for customer. | Pass |
| LIVE-03 | Increment View Count | 1. Send POST request to `/livestreams/{id}/view` twice. | View count increases by 2 in database. | Counter increased as expected. | Pass |
| LIVE-04 | Pin Product to Livestream | 1. Authenticate as admin user.<br>2. Send POST request to `/livestreams/{id}/pin-product` with productId in request body.<br>3. Send GET request to `/livestreams/{id}/pinned-products`. | 200 status; product appears in pinned products list. | Pinned list contained the product. | Pass |
| **Chat & Communication** |
| CHAT-01 | List FAQs | 1. Send GET request to `/chat/faqs` with optional query params (category, limit). | 200 status with list of FAQs filtered by category/limit. | 200 returned with expected structure. | Pass |
| CHAT-02 | AI Chat Response | 1. Ensure GEMINI_API_KEY is set in environment.<br>2. Send POST request to `/chat/ai` with message in request body. | 200 status with AI response and metadata fields present. | 200 returned; AI response provided. | Pass |
| CHAT-03 | Create FAQ Requires Auth | 1. Send POST request to `/chat/admin/faq` without token.<br>2. Repeat with admin token and valid body. | 401 status without token; 201 status with admin token. | Behaved as expected. | Pass |
| CHAT-04 | Staff Reply Requires Auth | 1. Send POST request to `/chat/admin/reply` without token.<br>2. Repeat with admin token. | 401 status without token; 200 status with admin token. | Behaved as expected. | Pass |
| **Payment Integration** |
| PAYMENT-01 | Create VNPay Payment URL (Customer) | 1. Authenticate as customer user.<br>2. Send POST request to `/payments/vnpay/create` with order info. | 200 status with VNPay payment URL for redirect. | 200 returned with VNPay URL. | Pass |
| PAYMENT-02 | Create VNPay Payment URL (Wrong Role) | 1. Authenticate as admin user.<br>2. Send POST request to `/payments/vnpay/create`. | 403 status due to role middleware requiring "customer" role. | 403 returned. | Pass |
| PAYMENT-03 | VNPay Return Handling | 1. Simulate GET request to `/payments/vnpay/return` with sandbox parameters. | 200 status and order status updated according to VNPay response. | Handled and returned expected structure. | Pass |
| **Security & Performance** |
| SECURITY-01 | Rate Limiting Applied | 1. Send > 1000 requests within 15-minute window to same endpoint. | 429 Too Many Requests status returned after threshold exceeded. | 429 returned when threshold exceeded. | Pass |
| **Static Assets** |
| STATIC-01 | Serve Uploaded Asset | 1. Send GET request to `/uploads/{existing-file}`. | 200 status with binary file response. | 200 returned file successfully. | Pass |
| **API Documentation** |
| DOCS-01 | Swagger UI Available | 1. Navigate to `/api-docs` in web browser. | 200 status with interactive API documentation interface. | Swagger UI loaded successfully. | Pass |
| **Frontend Features** |
| FRONTEND-01 | Locale Switch Updates UI | 1. Open application in browser.<br>2. Switch language using language selector (Vue i18n). | Visible strings update without page reload. | UI strings updated immediately. | Pass |
...
**Notes:**
- All API endpoints follow the pattern: `http://localhost:3000{endpoint}`
- Authentication tests use JWT tokens in Authorization header: `Bearer {token}`
- Role-based access control (RBAC) verified for admin-only endpoints
- File upload tests verify both success and error scenarios
- Rate limiting set to 1000 requests per 15 minutes per IP
