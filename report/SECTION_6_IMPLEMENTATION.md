...
The WrenCos system implements a dual-database architecture to support both transactional and AI-powered features. The physical database was created by translating the ERD entities into concrete database schemas using two complementary database management systems:

### MongoDB (Primary Operational Database)

**Database Management System:** MongoDB (via MongoDB Atlas)

MongoDB serves as the primary database, implementing all relational entities from the ERD. The schema was created by mapping each ERD entity to a Mongoose collection with the following approach:

- **Core Collections:** User, Category, Product, Order, OrderItem, LiveStream, ChatConversation, EmailCampaign, EmailTemplate, EmailSegment, EmailAnalytics, NewsletterSubscription, CashFlowTransaction, BusinessExpense, and Employee collections
- **Schema Implementation:** Each collection is defined using Mongoose schemas with field validation, type constraints, and enum restrictions matching the ERD attributes
- **Relationships:** Foreign key relationships are implemented using `ObjectId` references with automatic population via Mongoose's `ref` feature
- **Indexes:** Performance indexes created on frequently queried fields (isActive, createdAt, viewCount) and compound indexes for multi-field queries
- **Business Logic:** Pre/post hooks implement domain logic (e.g., bcrypt password hashing on User save, view count increments on LiveStream)

**Connection:** Established through Mongoose ODM with connection pooling, automatic retry writes, and write concern set to majority for data durability via MongoDB Atlas.

### Qdrant (Vector Database for AI)

**Database Management System:** Qdrant (Vector Database)

Qdrant was added as a specialized database for semantic search and AI dermatology recommendations. The vector database was created and populated as follows:

- **Collection Creation:** A single collection named `dermatology_knowledge` was initialized with 768-dimensional vectors (using Google Gemini's text-embedding-004 model) and cosine similarity distance metric
- **Document Indexing:** Knowledge base documents were split into 1500-character chunks with 300-character overlap, converted to embeddings via Gemini API, and batch-indexed into Qdrant (50 documents per batch with 3-retry fallback)
- **Search Mechanism:** Queries are embedded using the same Gemini model, then searched against the vector collection with a 0.4 similarity threshold to retrieve top-3 most relevant chunks
- **Docker Deployment:** Containerized via docker-compose.qdrant.yml with persistent storage at `./qdrant_storage`, dual API support (REST on port 6333, gRPC on port 6334), and health checks

### Integration

The two databases work together: MongoDB handles all transactional data (users, orders, livestreams, emails), while Qdrant enables the RAG (Retrieval-Augmented Generation) pipeline for AI dermatology responses by retrieving relevant knowledge chunks that are then passed to the Gemini AI model for context-aware answers.
