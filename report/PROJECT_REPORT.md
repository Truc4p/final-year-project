...
The comparison matrix evaluates platforms across five critical success factors derived from contemporary e-commerce literature. E-commerce functionality represents foundational capability requirements established by DeLone and McLean (2016) as essential for platform viability. Live streaming commerce emerges as a transformative engagement mechanism, with Wongkitrungrueng and Assarut (2020)demonstrating its significant impact on consumer trust and purchase intentions in social commerce contexts. AI personalization constitutes a critical differentiator, with Davenport et al. (2020) evidencing substantial conversion rate improvements through intelligent recommendation systems. Multi-platform support reflects the omnichannel imperative articulated by Verhoef et al. (2015), addressing customer expectations for seamless cross-device experiences. Implementation cost determines accessibility for SME retailers, a factor emphasized by Cao and Li (2015) as crucial for technology adoption decisions. Collectively, these five dimensions provide a comprehensive framework for evaluating platform suitability for SME beauty retailers, balancing functional capability, competitive differentiation, and economic feasibility.

| Feature / Aspect | Shopify | Adobe Commerce | Sephora Platform | Wrencos System (Proposed) |
|---|---|---|---|---|
| **Target Audience** | SMEs to mid-market retailers | Enterprise retailers | Beauty consumers (premium focus) | SME beauty retailers and beauty consumers |
| **Core Technology** | Proprietary SaaS cloud platform | Enterprise Java-based system | Proprietary web + mobile platform | Node.js/Vue.js/React Native full-stack |
| **E-Commerce Functionality** | Comprehensive product catalog, cart, checkout | Advanced inventory, multi-channel | Luxury product focus | Complete e-commerce with beauty-specific attributes |
| **Live Streaming Commerce** | Third-party integrations only | Limited native support | Integrated livestream shopping | Native WebSocket-based livestream |
| **AI Personalization** | Third-party apps (Klaviyo, Kenshoo) | Limited native AI | Proprietary recommendation engine | Native Google Gemini integration |
| **Multi-Platform Support** | Web only | Web only | Web + iOS/Android mobile | Web + iOS/Android mobile |
| **Implementation Cost** | USD 29–299/month SaaS | USD 100,000–500,000+ | Proprietary (not available) | Affordable cloud deployment |
...
**Historical Context and Evolution:** React, released by Facebook in 2013, pioneered the component-based architecture and virtual DOM paradigm, fundamentally transforming how developers conceptualize user interface construction (Majchrzak, Biørn-Hansen & Grønli, 2020). Vue.js emerged in 2014 as a progressive framework designed by Evan You, explicitly targeting ease of adoption whilst maintaining sophisticated capabilities for complex applications (You, 2019). Angular, originally released as AngularJS in 2010 and completely rewritten as Angular 2+ in 2016, represents Google's enterprise-oriented solution emphasizing comprehensive tooling and opinionated architecture (Haitz et al., 2023). The 2015–2025 period witnessed React establishing market dominance with approximately 42% adoption among professional developers, whilst Vue gained substantial traction in Asia-Pacific markets and Angular maintained strongholds in enterprise environments (Stack Overflow, 2024).

**Comparative Technical Analysis:** The comparison matrix synthesizes findings from Haitz et al.'s (2023) empirical study of framework performance in enterprise applications, supplemented by Majchrzak et al.'s (2020) comprehensive analysis of adaptive web application frameworks.
...
| **Ecosystem Maturity** | Extensive; robust state management (Redux, MobX), routing, UI libraries | Growing; Vue Router, Vuex/Pinia, extensive community plugins | Comprehensive; integrated CLI, routing, forms, HTTP client |
| **State Management** | External libraries required (Redux, Context API, Zustand) | Native Vuex/Pinia with intuitive API | Built-in services and RxJS observables |
...
| **Bundle Size** | ~40 KB (minified + gzipped) | ~20 KB (minified + gzipped) | ~500 KB+ (comprehensive framework) |
...
**Critical Evaluation and Project-Specific Justification:** Haitz et al. (2023) demonstrate that React and Vue.js achieve comparable runtime performance in enterprise applications, with both frameworks outperforming Angular in initial rendering speed by 15–20% due to lighter bundle sizes.

For the Wrencos platform, Vue.js was selected for web frontend development based on four critical factors. First, **development velocity and learning curve**: Vue's progressive nature and intuitive template syntax enable rapid prototyping whilst accommodating complexity growth, particularly valuable for SME-oriented platforms requiring iterative development (You, 2019). The template-based approach reduces cognitive overhead compared to React's JSX, facilitating faster onboarding for developers with traditional HTML/CSS backgrounds. Second, **bundle size and performance**: Vue's 20 KB core represents 50% smaller footprint than React, directly impacting initial load times critical for e-commerce conversion rates where Nielsen Norman Group research demonstrates that each additional second of load time reduces conversion by 7% (Majchrzak et al., 2020). Third, **state management integration**: Vue 3's Composition API with Pinia provides native, intuitive state management without requiring external dependencies, reducing architectural complexity whilst maintaining sophistication necessary for managing e-commerce state (cart, authentication, real-time updates). Fourth, **real-time capabilities**: Vue's reactive data binding integrates seamlessly with WebSocket infrastructure required for live streaming commerce, enabling efficient real-time UI updates without manual DOM manipulation (Haitz et al., 2023).

React was deliberately not selected despite its larger ecosystem because the incremental benefits of additional third-party libraries did not justify the steeper learning curve and larger bundle size for an SME-targeted platform prioritizing accessibility and performance. Redux state management, whilst powerful, introduces substantial boilerplate code and architectural complexity unnecessary for Wrencos's requirements, which Pinia addresses more elegantly. Angular was excluded due to its substantial bundle size (500 KB+) and steep learning curve requiring TypeScript and RxJS expertise, representing inappropriate complexity overhead for the project scope (Haitz et al., 2023). The comprehensive nature that benefits large enterprise teams with dedicated frontend specialists becomes a liability for SME-oriented development requiring lean, maintainable codebases.
...
**Summary:** The technology selection demonstrates evidence-based decision-making balancing theoretical performance benchmarks from academic literature (Haitz et al., 2023; Majchrzak et al., 2020) with practical considerations of ecosystem maturity, developer experience, and project-specific requirements. Vue.js for web frontends and React Native for mobile applications represent optimal choices for an SME-oriented e-commerce platform requiring rapid development, excellent performance, and maintainable architecture, justified through systematic comparative analysis rather than popularity-driven selection.
